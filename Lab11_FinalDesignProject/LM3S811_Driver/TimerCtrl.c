...
void Timer1B_Handler(void) {
	...
	if(forward) {
		if(GPIOPinRead(GPIO_PORTB_BASE, GPIO_PIN_1)) {
			...
			TimerLoadSet(TIMER1_BASE, TIMER_B, 10000-(100*speed));
		} else {
		  	GPIOPinWrite(GPIO_PORTB_BASE, GPIO_PIN_1, 0xFF);
			GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_0, 0);
			TimerLoadSet(TIMER1_BASE, TIMER_B, (100*speed));
		}				  
	} else {
...
void Timer2A_Handler(void) {
	...
	if(left) {
		if(GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_1)) {
			...
			TimerLoadSet(TIMER2_BASE, TIMER_A, 10000-(100*steering));
		} else {
		  	GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_1, 0xFF);
			GPIOPinWrite(GPIO_PORTB_BASE, GPIO_PIN_0, 0);
			TimerLoadSet(TIMER2_BASE, TIMER_A, (100*steering));
		}				  
	} else {
